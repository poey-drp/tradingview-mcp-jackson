...
5. **ตาราง probability:** **ACTION**, **Entry**, TP1, TP2, TP3, SL — ราคาเป๊ะ + ระยะ + % โอกาสแตะ + R:R + **ETA (จำนวนแท่ง + เวลาโดยประมาณ)** + **time-stop (เส้นตายเป็นจำนวนแท่ง)**
   - **แถวแรกต้องเป็น ACTION เสมอ** = คำสั่งสำหรับ ณ ราคาปัจจุบัน: **BUY / SELL / WAIT** (ตัวหนา) + เหตุผลสั้นๆ ในช่องถัดไป (เช่น ราคาปัจจุบัน + ทำไมถึงสั่งแบบนั้น) — ช่องที่เหลือใส่ `—`
     - `BUY`/`SELL` = ราคาตอนนี้เข้าได้เลย; `WAIT` = ยังไม่เข้า ระบุเงื่อนไข trigger ที่จะเปลี่ยนเป็น BUY/SELL
   - **แถวสองเป็น Entry** (entry zone ราคาเป๊ะ; ช่อง ระยะ/%/R:R/ETA ใส่ `—`) — ใช้ราคากลางโซน entry เป็นฐานคำนวณระยะของ TP/SL ทุกตัว
   ...
   - รูปแบบตาราง (ACTION แถวแรก, Entry แถวสอง):
     - `🟡 ACTION ตอนนี้ | WAIT | ราคา 24,540 อยู่กลางโซน รอเลือกข้างที่ 24,500 | — | — | — | —`
     - `Entry 24,530–24,535 | — | — | — | — | รอย่อเข้าโซน`
     - `TP1 24,560 | +30 | ~68% | R:R 1.5 | ETA ~3 แท่ง (~15m) | time-stop 6 แท่ง`
